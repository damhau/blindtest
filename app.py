...
# Global OAuth service without cache - only used for initial auth flow
spotify_oauth_service = get_spotify_oauth_service(use_cache=False)
openai_service = get_openai_service()


def get_user_oauth_service(user_id=None):
    """Get user-specific OAuth service or fallback to global service"""
    if user_id:
        return get_spotify_oauth_service(user_id=user_id)
    return spotify_oauth_service
...
class Room:
    ...
    def add_participant(self, sid, name):
        self.participants[sid] = {"sid": sid, "name": name, "score": 0}
    ...
    def get_scores(self):
        return sorted(self.participants.values(), key=lambda x: x["score"], reverse=True)
...
def callback():
    ...
    # Get user ID to create user-specific cache
    try:
        temp_client = spotipy.Spotify(auth=token_info["access_token"])
        user_info = temp_client.current_user()
        user_id = user_info["id"]

        # Create user-specific OAuth service and save token to user-specific cache
        user_oauth_service = get_spotify_oauth_service(user_id=user_id)
        user_oauth_service.sp_oauth.cache_handler.save_token_to_cache(token_info)
...
def get_user_profile():
    ...
    user_oauth = get_user_oauth_service(user_id)
    ...
    try:
        ...
        sp_client, refreshed_token = user_oauth.get_spotify_client(token_info)
        if not sp_client:
...
def logout():
    ...
    # Clear user-specific cache if we have a user_id
...
def get_spotify_token():
    ...
    return jsonify(
        {"access_token": token_info.get("access_token"), "expires_in": token_info.get("expires_in")}
    )
...
def my_playlists():
    ...
    if not user_oauth:
        return jsonify({"error": "OAuth service not available"}), 500
    ...
    for pin, room in rooms.items():
        if room.host_sid == sid:
            ...
            socketio.emit(
                ...
                {"name": participant["name"], "participants": list(room.participants.values())},
                room=pin,
            )
            break
...
def handle_join_room(data):
    ...
    response_data = {"pin": pin, "name": name, "participants": list(room.participants.values())}
    ...
    if was_host:
        ...
        if old_sid:
            ...
            if room.question_index in room.answers and old_sid in room.answers[room.question_index]:
                room.answers[room.question_index][request.sid] = room.answers[room.question_index][
                    old_sid
                ]
            ...
            emit(
                "rejoin_success",
                {"success": True, "current_score": current_score, "state": room.state},
            )

            # Notify others about the reconnection
            socketio.emit(
                "participant_reconnected",
                {"name": name, "participants": list(room.participants.values())},
                room=pin,
                skip_sid=request.sid,
            )
...
def handle_start_game(data):
    ...
    if room.token_info and spotify_oauth_service:
        ...
        tracks, error = spotify_oauth_service.get_playlist_tracks(
            sp_client, room.playlist_id, limit=200, fetch_pool_size=200
        )
    ...
    if not tracks:
        emit(
            "error",
            {"message": "No tracks found in playlist. Try a different playlist with more songs."},
        )
        return
    ...
    if len(tracks) < total_songs_needed:
        emit(
            "error",
            {
                "message": f"Not enough tracks in playlist. Need {total_songs_needed} but only found {len(tracks)}. Try a larger playlist or reduce songs/games."
            },
        )
        return
    ...
    if not room.token_info and tracks_with_audio == 0:
        emit(
            "error",
            {
                "message": "No tracks in this playlist have audio previews. Please login with Spotify or try a different playlist."
            },
        )
        return
    ...
    if openai_service:
        ...
        for i, track in enumerate(tracks):
            ...
            for cand in candidates:
                ...
                if (
                    cand
                    and _norm_name(cand) != _norm_name(correct)
                    and artist_exists_on_spotify(cand)
                ):
        ...
        if needs_repair:
            ...
            try:
                ...
                for local_i, original_i in enumerate(repair_map):
                    ...
                    target_real = (
                        3
                        if (original_i < len(funny_enabled) and not funny_enabled[original_i])
                        else 2
                    )
    ...
    for idx, track in enumerate(tracks, 1):
        ...
        real_candidates = (
            gpt_real_distractors[idx - 1] if idx - 1 < len(gpt_real_distractors) else []
        )
        real_picks = []
...
def send_question(pin):
    ...
    room.question_start_scores = {sid: player["score"] for sid, player in room.participants.items()}
...
def handle_submit_answer(data):
    ...
    emit(
        "answer_feedback",
        {"correct": is_correct, "your_score": room.participants[request.sid]["score"]},
    )
    ...
    if room.question_start_time:
        ...
        server_response_time_ms = int(
            (server_received - room.question_start_time).total_seconds() * 1000
        )
        ...
        if (
            client_response_time_ms is not None
            and isinstance(client_response_time_ms, (int, float))
            and client_response_time_ms > 0
        ):
    ...
    while True:
        ...
        for pin, room in rooms_to_check:
            ...
            for sid in sids_to_remove:
                ...
                socketio.emit(
                    "participant_left",
                    {"participants": list(room.participants.values()), "scores": room.get_scores()},
                    room=pin,
                )
